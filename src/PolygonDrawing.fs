...
let updateModel (msg: Msg) (model: Model) =
    ...
    | AddPoint _ ->
        let newModel =
            ...
            | None, Some pos ->
                { model with
                    currentPolygon = Some [ pos ] }
            | Some polyLine, Some pos ->
                { model with
    ...
    | FinishPolygon ->
        let newModel =
            match model.currentPolygon, model.finishedPolygons with
            | None, _ -> model
            | Some current, polygons ->
                { model with
                    currentPolygon = None
                    finishedPolygons = current :: polygons }
...
let addUndoRedo (updateFunction: Msg -> Model -> Model) (msg: Msg) (model: Model) =
    ...
    | Undo ->
       ...
       match model.past with
        | Some past -> {past with future = Some model}
        | _ -> model 
...
let render (model: Model) (dispatch: Msg -> unit) =
    let border =
        Svg.rect
            [ // i used ; to group together attributes semantically.
              svg.x1 0
              svg.x2 500
              svg.y1 0
              svg.y2 500
    ...
    let finisehdPolygons = model.finishedPolygons |> List.collect (viewPolygon "green")
    ...
    Html.div
        [ prop.style [ style.custom ("userSelect", "none") ]
          prop.children
              [ Html.h1 "Simplest drawing by Rössler & Yarar"
                Html.button
